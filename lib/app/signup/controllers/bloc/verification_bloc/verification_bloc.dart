  ...
  Future<void> _verificationEvent(event, emit) async {
    if (event is OtpChangedEvent) {
      ...
      try {
        ...
        isArrive = event.signUp? await CheckCodeService().checkCode(emailModel: EmailVerificationModel(email: event.emailModel.email, code: event.emailModel.code))
        :await CheckCodeForgetPasswordService().checkCode(emailModel: EmailVerificationModel(email: event.emailModel.email, code: event.emailModel.code));
        if (isArrive) {
          emit(VerificationSuccess(emailModel:EmailVerificationModel(email: event.emailModel.email, code: event.emailModel.code)));
        } else {
